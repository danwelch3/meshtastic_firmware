...
public:
  ...
  public:
    ...
    void initialize( int8_t csPin, SPIClass &spiInterface = SPI, uint32_t spi_freq = SPI_DATA_RATE);
    void initialize( uint8_t address = MPU9250_ADDRESS_AD0, TwoWire &wirePort = Wire, uint32_t clock_frequency = 100000 );
    bool testConnection();
    void getMres();
    void getGres();
    void getAres();
