...
#define BMX160_I2C_ADDR 0x69             //< bmx160 I2C address */
...
#define BMX160_GET_BITS(regvar, bitname) ((regvar & bitname##_MSK) >> bitname##_POS)
#define BMX160_SET_BITS(regvar, bitname, val) ((regvar & ~bitname##_MSK) | ((val << bitname##_POS) & bitname##_MSK))

#define BMX160_SET_BITS_POS_0(reg_data, bitname, data) ((reg_data & ~(bitname##_MSK)) | (data & bitname##_MSK))
...
typedef struct {
    ...
    uint16_t length;        /**< While calling the API  "BMX160_get_fifo_data" , length stores number of bytes in FIFO to be read
                               (specified by user as input) and after execution of the API ,number of FIFO data bytes available is
                               provided as an output to user */
...
typedef struct {
    ...
    eBmx160AnySigMotionActiveInterruptState_t
        any_sig_sel; /**< Hold active interrupts status for any and sig motion 0 - Any-motion enable, 1 - Sig-motion enable,  -1
                        neither any-motion nor sig-motion selected */
    sBmx160Cfg_t magnCfg;      /**< Structure to configure Magnetometer sensor */
    sBmx160Cfg_t prevMagnCfg;  /**< Structure to hold previous/old magn config parameters.This is used at driver level to prevent
                                  overwriting of same data, hence user does not change it in the code */
    sBmx160Cfg_t accelCfg;     /**< Structure to configure Accel sensor */
    sBmx160Cfg_t prevAccelCfg; /**< Structure to hold previous/old accel config parameters.This is used at driver level to prevent
                                  overwriting of same data, hence user does not change it in the code */
    sBmx160Cfg_t gyroCfg;      /**< Structure to configure Gyro sensor */
    sBmx160Cfg_t prevGyroCfg;  /**< Structure to hold previous/old gyro config parameters. This is used at driver level to prevent
                                  overwriting of same data, hence user does not change it in the code */
...
typedef struct {
    ...
    uint16_t outputType : 1; /**< 0 - active low , 1 - active high level.if outputEn is 1,this applies to interrupts,else
                                PMU_trigger */
...
typedef struct {
    ...
    uint16_t
        flatHoldTime : 2; /**< delay time for which the flat value must remain stable for the flat interrupt to be generated */
