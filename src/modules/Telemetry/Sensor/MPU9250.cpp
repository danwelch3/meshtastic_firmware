...
void MPU9250::initialize(int8_t csPin, SPIClass &spiInterface, uint32_t spi_freq)
{
    // Use hardware SPI communication
    // If used with sparkfun breakout board
    // https://www.sparkfun.com/products/13762 , change the pre-soldered JP2 to
...
void MPU9250::initialize(uint8_t address, TwoWire &wirePort, uint32_t clock_frequency)
{
    _I2Caddr = address;
    _wire = &wirePort;
    _spi = NULL;
...
uint8_t MPU9250::readMagByteSPI(uint8_t registerAddress)
{
    ...
    uint32_t count = 0;
    while (((I2C_MASTER_STATUS & 0b01000000) == 0) &&
           (count++ < 100000)) // Checks against the I2C_SLV4_DONE bit in the I2C master status register
    {
...
uint8_t MPU9250::writeMagByteSPI(uint8_t registerAddress, uint8_t data)
{
    ...
    uint32_t count = 0;
    while (((I2C_MASTER_STATUS & 0b01000000) == 0) &&
           (count++ < 10000)) // Checks against the I2C_SLV4_DONE bit in the I2C master status register
    {
        I2C_MASTER_STATUS = readByteSPI(54);
    }
    if (count > 10000) {
